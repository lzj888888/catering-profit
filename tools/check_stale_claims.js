...
const PHRASES = [
  ...
  {
    // 🆕 round75（2026-09-21）：**判据类**（区别于上面三条「能力类」）——
    //    上面都是"平台能不能"，这条是"我上一轮自己下的结论错了"，且错了整整三天没人发现。
    //    模式刻意收紧到必须**同时**出现「月份」(只有|只给)(两个|2个) 与「设计/不是 bug」
    //    ⇒ 只命中"把现象判成设计"的断言，不误杀"复述用户反馈原文"（.js 注释里那句没有判词）。
    id: '月份只有两个=设计',
    re: /月份[^。\n]{0,12}只有(两|2)个[^\n]{0,30}(设计|不是\s*(bug|缺陷|问题)|正常现象)/,
    why: 'round57 把李老师真机反馈「月份选择只有两个月份」判成"这是设计不是 bug" —— 2026-09-21（round75）实锤为'
       + '**真缺陷**：月份下拉先前只给「已建档月份 ∪ 当月」⇒ 新店首次只剩当月、想补录往月连选项都没有。'
       + '正解 = 滚动窗口 ∪ 已建档 ∪ 当月（`utils/ui.recentMonths`，单源）。'
       + '⚠️ 命中行须带引述标记（如「曾」）：记录"曾判成设计"是留痕，不是活断言。',
  },
];

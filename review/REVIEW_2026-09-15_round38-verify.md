...
<!-- 以下为 2026-09-22 round97 追加（本区最后一条原本停在 2026-09-19 02:50） -->
- [2026-09-22 15:45] ✅ **round97 已落**（李老师「按你说的处理」= 批准 round96 §6 的 T1~T6）· 落地 **T4 + T4b + T3′ + T1**（T2 经复核**证伪放弃**：快速模式无补贴数据源 ⇒ A.11.4 配平等式依赖 `subsidy` ⇒ 硬套**稳定误报**）· **T5/T6 待李老师**（堂食折扣口径一句话 / 一份脱敏月度账单）· 分析件 `review/NOTE_2026-09-22_round97-expense-preset-and-mk-by-platform.md` · 证据：`node verify_all.js` → **94/94 套件通过 RC=0**（`review/evidence/gate_94_t4.txt` + `gate_94_t1.txt`）；变异回灌 **13 组 0 异常**（`review/evidence/mut_r97_t1.txt`，10 组该红已红+点名、3 组反向证据该绿保持绿、13 组全部逐字节还原）· 新增守卫 `tools/check_expense_item_seed.js`（R121：费用项清单**四副本**逐项有序比对，同族病**第 25 例**）⇒ 套件 93→94 · commit `7d3960e` + `8f541e2`
- [2026-09-22 15:45] ⚠️ **流程空档登记（请复审方裁定）** · 本区最后一条原本停在 **2026-09-19 02:50**，此后 **09-20 ~ 09-22 的 round57 ~ round97 未再往本区追加**。原因是本仓**最近一份 `REVIEW_*` 就是本件（round38）**，而 `review/README.md §7 第 2 条`要求「回执追加到**对应** `REVIEW_*` 的 §3」⇒ **无新件时无处可追加**，该期间的交接一律以 `NOTE_*` 文件承载（R96 已定「执行方分析件 = `NOTE_*`」）。本轮按条文**补回一行**，并把空档如实登记在此 —— 请裁定「是否应在新 `REVIEW_*` 里重新开 §3 区」，或改条文为「无新件时回执落 `NOTE_*` 即可」。

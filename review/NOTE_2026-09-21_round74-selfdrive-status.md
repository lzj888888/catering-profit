...
- [2026-09-21 13:08] R74 **失误（我方，如实记）** · `git commit -m ...`（**未带 pathspec**）把并发方**已暂存**的 3 件在途改动
  （`pages/month/index.js` / `tools/selftest_ui_fix.js` / `utils/ui.js`）一并提交进 `9ddee3e`。
  **内容未丢失**（对方工作树仍在其上继续改，提交后 `utils/ui.js` 又出现新的未暂存改动），
  且提交前门禁在其上 **81/81 RC=0** ⇒ 不是坏快照；但**违反「不代提交并发方产出」**。
  根因＝我刚在 §2.1 写下「不能假设暂存区归自己」，却在最后一次提交时**只 `git status` 没复核 `git diff --cached --stat`**。
  ⇒ 硬改法（已写进重启键 round74 条）：**本仓提交一律带 pathspec**（`git commit -- <path>`），或提交前必看 `git diff --cached --stat`。
  已推送远端，**不强推回滚**（强推是我们自己的红线）；如李老师要求回滚，走 `git revert` 而非 reset。

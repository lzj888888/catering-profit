...
### §6.29 R77 / R78 / ③（2026-09-17 傍晚）—— 「引用位置必须可解析」独立成守卫 + A6 同源落点交叉引用（套件 55→56）

**来源**：复审方 **round31（R77）** + **round32（R78）**；round31 的 R75/R76/③ 见 §6.28。本轮已取件入库 `review/REVIEW_2026-09-15_round32-verify.md`（`review/` 35 → 36 份），并按其 §3 追加 9 条回执。

**R77 的根因定性（先核事实再动手，纠正复审方的表述）**
- 复审方在 round32 §2.5 查了三条候选路径，得出「报告链**事实中断**」。**我复核后：三条候选的实测结果与他完全一致，但定性要改** —— 真报告目录 `C:\Users\lzj\WorkBuddy\2026-09-08-22-08-11\.workbuddy\memory\` **一直在按日更新**（`ls -1` 实测 **10 份**日报 09-08~09-17，其中他点名的 `2026-09-16.md` 存在：**473 行 / 67428 B**）。
- ⇒ 这**不是"报告缺失"，是"协议给了一个不可解析的路径、且同名处没有说明身份"**。复审方查的两个"看得到文件"的地方都是**诱饵**：`~\.workbuddy\memory\`（自动注入的 `<uuid>_memory.md` = **云端画像缓存**）、`~\.workbuddy\MEMORY.md`（**跨项目用户级记忆**）。
- ⇒ 结论升级为一条**通用纪律**（已写进 `review/README.md §7` 第 14 条）：**不可解析的引用会把"我核不了"升级成"你断链了"**。

**处置 ①：四处活副本改绝对路径（先 `grep` 清点，再动手）**
- 实测报告路径的**活副本 = 4 个文件 / 5 行**：`review/README.md` **L14**（§1 表格）+ **L163**（§7 第 14 条）+ `SMOKETEST_RUNBOOK.md` **L177**（桥接行）+ `specs/dev-specs/★知识存储点_2026-09-10.md` **L168**（§1.1 ⑪，本轮新建）+ `新手上云操作手册.md` **L250**（§10.5）。
- ⚠️ 最后那处（手册 §10.5）**没有任何人提过**，是执行「改一处须 grep 查全同类」时扫出来的 —— 若不扫，它会成为唯一的旧写法残留。另有 **2 行 `.txt` 派生物**随门禁 K 组整链重生。
- **选"绝对路径"而非"移进仓库"**：移进仓库会**新增第二份副本**（正是 R74/R75/R76 三轮在消灭的病），且日报是 80KB×N 的大文件；改协议文本即可达成同等可核性（复核方与我同机，直读绝对路径）。**历史流水（round16~19 回执、复审方 round32 §2.5）按"只增不改"不动。**

**处置 ②：新建守卫 `tools/check_handoff_paths.js`（P1~P6，套件 55→56）**
- 判据：**P1** 协议锚点行（`WorkBuddy 报告`）必须给出可解析绝对路径（fail-closed：锚点行找不到即判红并说明"表结构变了，判据失效"）· **P2** 两个诱饵必须被点名**且标注紧随其后** · **P3** 活文档中**每一处**相对写法须"同行可解析"或"紧邻标注"（**逐处判定**）· **P4** 两层：解析到的路径**都真实存在** + **凡提到报告路径的活文档都必须给出可解析绝对路径** · **P5** 目录里 ≥1 份日报（打印最新 mtime）· **P6** 诱饵标注**不许腐**（存在性须被核实）。
- **核心是 P4**：此前**从没有人验证过这个路径真的存在** —— 这是它写错了也一路全绿的唯一原因。
- ⚠️ **硬化后守卫在基线态就抓到 3 处真问题**（含我本轮新写的两处）：① Git-Bash 形态 `/c/Users/…/.workbuddy/memory`（**正确的定位命令**）被误判成裸相对写法 ⇒ 加 `MSYS_ALL` 识别；② §7 第 14 条本身引用了裸相对写法 ⇒ 加标注；③ **重启键 ⑪ 提到报告路径却无可解析绝对路径** ⇒ 已补。

**⚠️ 主动自曝：落地时守卫自身有三处缺陷，全部"改文本"防不住**
1. **同一个 `/g` 正则既 `.test()` 又 `matchAll()` 会串 `lastIndex` 状态** ⇒ 实测造成一次**假阴性**（RUNBOOK 那行被漏判，且 P1 断言"通过"却打印失败详情）⇒ 拆成无 `/g` / 带 `/g` **两个实例**（注释里写明"别退回一个"）。
2. **标注窗口 80 字符过宽**：两个诱饵写在同一行时，会把"**隔壁诱饵的标注**"算进来 ⇒ 删掉本诱饵的标注**仍判绿** ⇒ 收紧为 `LABEL_WINDOW = 40`，教训写进常量旁注释。
3. **两处判定过松**：P3 曾"任一处有标注就 `return` 整行"（会放过同行的**另一处**裸相对写法）；P4 的存在性检查会被**前缀匹配**蒙过（尾巴写坏成 `…\memory_nope` **仍判绿**）⇒ P3 改逐处判定 + "落在绝对路径内部"豁免；ABS 正则加尾部 **lookahead `(?![A-Za-z0-9_])`** 让坏后缀**解析不出来**，再由 P4 文档级断言点红。

**变异回灌 6 组**（同 §6.27 流程：`cp` 备份 → 改 → **自证变异已生效** → 跑判据 → `cp` 还原 → `md5` 逐字节比对；全程**零删除**）

| # | 变异 | 结果 |
|---|---|---|
| M13 | 绝对路径 → 仓库相对写法 | ❌ **P1+P3+P4 三红**（点名 `review/README.md:14`） |
| M14 | 路径尾巴写坏 `memory` → `memory_nope` | ❌ **三红**；⚠️ **加固前这一组不转红**（前缀匹配被蒙过）⇒ 本轮**变出来的真洞** |
| M15 | 删掉诱饵①的整段标注 | ❌ P2① + P3 红 |
| M16 | **恒真探针**：诱饵解析多套一层不存在的目录 | ❌ **仅 P6 红 ×2、P2 仍绿** ⇒ 证明 P6 **不是恒真** |
| M17 | **反向证据**：正斜杠等价形态 | ✅ **保持 11 通过 / 0 失败** ⇒ 不假红 |
| M18 | 往 RUNBOOK 插入一行裸相对引用 | ❌ P3 点名 `SMOKETEST_RUNBOOK.md:175` |

**R78：在案项 A6「无记录就自动建档」的同源第二处**
- 复审方只点名 `common/auth.js::resolveAuth` 一处；我按"修一处须 grep 查全同类"实测 `grep -rn "collection('shop')\.add" cloudfunctions/` ⇒ **现存两处**：① 单源 `common/auth.js::resolveAuth`（派生进 **44 个** `cx_auth.js`）；② **`getShopContext/index.js:31-41` 内联**。
- 处置：A6 条目末尾补"**同源第二处**"+ 双向回指（`tools/check_idempotency.js` 的 `EXEMPT_WRITE[getShopContext]` 理由 ↔ 在案项 **A6**），并写明"**修并发建档必须两处一起改**"。
- 采纳复审方"**不建议为此新增判据**"：这是**并发缺陷**、不是**幂等缺陷**，硬塞进幂等守卫会让判据名不副实。
- 顺带补一条**他没提的差异证据**：第二处连**字段都不同**（`我的店铺` vs `默认店铺`、**缺 `is_deleted:false`**）⇒ 两处产出"看起来不同、实则同类"的重复 shop，按 A6 单点修会留下**更难发现**的另一处。⚠️ 同时区分：`cx_auth.js` 那 44 处是**派生副本**、不是第 45 处落点（否则会误导下一个人去改 44 个文件）。

**③（观察项）：S4 那条断言收敛为「计数类期望值」**
- 断言更名为「`verify_docx.py` 无写死的**计数类**期望值」，并在紧邻处注明 `OLD_STRINGS` 里的 `8 套件` 属"**被禁旧串**"（性质是**禁值**、不是**期望值**）⇒ 与本断言不冲突。
- 采纳复审方判断：**行为本来就是对的**（该断言只覆盖 `N 条索引` / `N 条是 unique` 两类模式），只是名字会让人以为矛盾而去"修"它 ⇒ **改名字，不改行为**。

**⚠️ 口径更正（自曝一处我自己写错的"更正"）**
- 我在回执初稿里写过「§6.27 曾记『活副本 6 处』，本轮实测报告路径活副本为 4 处」。**发布前自查回读原句，发现这是错的**：§6.27 的那句是 **R74 的建库口径** —— `3 处活副本（collections.js / init_db.js / 工序清单对照表）+ 2 处计数声明（手册、工序清单）+ 1 处自检脚本期望值` ⇒ 与**报告路径**不是同一个事实，**不能相减**（round31 §33 的"本轮 grep 出 6 处活副本"同理，也指**索引计数**链）。回执原件已按实测改写并保留撤回声明。
- 真正需要声明的更正只有一条、且 **§6.28 已记**：§6.27 该口径里**最后一项「1 处自检脚本期望值（`verify_docx.py`）」已被 R75 取代**（期望值派生化 ⇒ 不再存在第二份数字）。此处**回指、不重复声明**（台账只增不改）。

**验证终态**（逐项直跑）：门禁 A–L `GATE=0` · 形状守卫 **42/42** + `Δ(SUITES) 双向为零` + 1 WARN（`batch0` 的 `R71-ok`）· 幂等守卫 **54/0** · 建库单源守卫 **64/0** · **交接面守卫 11/0** · `check_requires` **0** · `verify_docx` **0**（3 份）· 全闸 **56/56**（`✅ [suite-count] ≡ SUITES.length = 56`）· `grep -rn MUTATION` 无命中、无 `.bak/.off` 残留 · 工作树干净 · **两方一致 = `732bdd3`**（`git ls-remote origin dev` == `git rev-parse HEAD`）。套件数三处人工面（`verify_all.js` 头部 + 重启键两处）已同步 55→56。

- [已落] **R77**（引用位置可解析 + 点名诱饵 + 机械化 `check_handoff_paths` P1~P6）· **R78**（A6 同源第二处 + 双向回指）· **③**（断言更名）
- [未落 · 待人工] ① **先补 40 条索引（现线上 1/40）** —— 其中 `audit_log.idx_audit_idem` 缺了会让幂等预检**全表扫描** ② 真云三验 ③ `ADMIN_SETUP_TOKEN` 的**值** ④ 上线三项（隐私政策 URL【硬阻塞】/ 审核测试账号 / 营业执照商户号）、`wechatide` 授权、R45 iOS 过滤
- [待复审] **R77/R78 待 round33 复核**（本轮 conditional 走"HEAD 已前进"分支：`31e3b5a` → **`732bdd3`**）；**R75/R76 亦可同批复跑**；**R72/R73 仍待专审**（范围见 round31 §2.4）

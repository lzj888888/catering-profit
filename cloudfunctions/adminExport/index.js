// cloudfunctions/adminExport/index.js —— 批次 6 · 数据导出（adminAuth + 角色控权）
//
// ⚠️ 导出权限（批次 6 §2.9）：**仅超管可导出全量数据，运营仅可导出订单明细**。
// ⚠️ 所有导出操作写 audit_log（导出人/时间/数据范围）。
// ⚠️ CSV 生成在 Service 侧（本文件内联纯函数 csvEscape/csvFromRows），返回 CSV 文本 + 文件名；
//   前端（H5）用 Blob 下载。金额分整数仅格式化展示。
...
exports.main = async (event) => {
  ...
  if (scope === 'entitlements') {
    // 全量权益明细（仅 super）
    const res = await db.collection('shop_entitlement').limit(1000).get();
  ...
  // 订单明细（超管/运营均可）
  const res = await db.collection('shop_payment_flow').where(cond).limit(2000).get();
  const rows = (res && res.data) || [];

 ...
 * @param {object} [opts] 测试注入 { mutateNoTail } ：mutateNoTail=true 时末月**不做尾差倒挤**，
 *                        仅用于 selftest「变异回验」证明判据有鉴别力；线上绝不传。
 * @returns {number} 摊销额分（区间外 = 0）
 */
function amountForMonthFen(asset, month, opts) {
  ...
  // ===== 末月（k === N）=====
  if (opts && opts.mutateNoTail) return base; // 变异注入：末月沿用 base（判断是否有鉴别力的对照）
  if (end === naturalEnd) {
 ...
 */
function calcAmortize(assets, targetMonth, opts) {
  const list = (assets && Array.isArray(assets)) ? assets : [];
  const details = [];
  let total = 0;
  for (const asset of list) {
    const amountFen = amountForMonthFen(asset, targetMonth, opts);
    total += amountFen;
    details.push({
...
function calcAmortizeSchedule(asset, opts) {
  const { start, end, naturalEnd } = getRangeIndex(asset);
  const rows = [];
  let sum = 0;
  for (let mi = start; mi <= end; mi++) {
    const m = formatMonthIndex(mi);
    const amount = amountForMonthFen(asset, m, opts);
    sum += amount;

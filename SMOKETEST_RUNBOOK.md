...
- 怎么改（三步，别只把 `smokeTest` 调到 10s 就收工）：① 定值 ② 单点试「`config.json` 加 `"timeout"` → 重发 → `info` 回读」③ 生效则推广成单源，否则退控制台手改并把值写进 `core/13_上线前查缺补漏…§5`。
...
- ✅ **正确做法**：云函数 → `smokeTest` →「版本与配置」把**超时时间调到 ≥10 秒**；改完用 `cli cloud functions info` 的 **`timeout` 列回读确认**（这是唯一能证明"已生效"的判据）。

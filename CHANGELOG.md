...
## 2026-09-25 · v1.9（云端测试通道 + 三条新坑 + 补交 v1.8 之后的未提交成果）
**背景**：round124 要在**真云上**补种一行配置（`feature_permissions.plan_free::free_quota`），却先卡在"IDE 起不来 → 坐标点不中 → 点击被模态框吃掉"三连；过程中还发现技能仓 **v1.8 之后的大量成果一直没提交**。
- 新增 `references/recipes.md` **配方 15**：云开发控制台 → 云函数 → **云端测试**全链路（**免部署、在真云上验逻辑**）。含实测坐标：∞ 图标 (1429,25) / 左侧「云函数」(166,375) / 搜索框 (1685,240) / 「云端测试」(1538,388) / 编辑器 (1200,400) / 绿色「运行测试」(1733,592)；入参出参四条纪律；**用"两次运行的结果差"证幂等**；以及"云端测试通过 ≠ 端到端通了"的边界说明。
- 新增 `references/pitfalls.md` **P33/P34/P35**：
  - **P33** IDE 的存活挂在 cli 的 WebSocket 上（日志末行恒为 `cli ws client disconnect`，断连即退）⇒ 续命配方 = 后台循环喂 `cli open`（**实测保活 9m48s / 30 轮全成功**）+ 副作用警告（反复重载会逼出 P35）。
  - **P34** 两套 DPI 口径（`win_gui`=物理像素 / 裸 ctypes 枚举=逻辑像素，150% 缩放下差 1.5×）+ **PrintWindow 主题失真** ⇒ **点击坐标只从全屏 1:1 PNG 取**，窗口 PNG 只用于阅读。
  - **P35** 模态框「模拟器长时间没有响应」挡住点击、而背景重绘让 hash 一直在变 ⇒ **修正 P22 的用法：hash 只可证否，不可证成**。
- `SKILL.md` 新增硬教训 **27/28/29**（同三条的精简版 + 指针）；硬教训 22 补上"md5 只可证否"的修正；配方索引补 配方 15。
- 🔧 **修正既有结构缺陷**：硬教训 16–26 此前被追加在 `## 移植到别的电脑` **之后**（编号列表被腰斩成"1–15 + 16–26 分居两处"）⇒ 已移回「硬教训」段内，**内容一字未改**，位移由脚本自校验顺序（15@102 → 16@104 → 26@125 → 安全护栏@131 → 移植@172）；`pitfalls.md` 第七节加注"P21 起为持续追加，请以条目标题判断类目"。
- 📦 **补交 v1.8 之后一直未提交的成果**（9-17～9-21 各会话产出，此前一直躺在工作区，一次 `git checkout` 即失）：
  `scripts/examples/`（`cloudbase_index` 建索引脚本族 / `cloudbase_deploy` 部署 runbook）、P25–P32、硬教训 21–26、配方 14.1。

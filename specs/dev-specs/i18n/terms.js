...
const TERMS = {
  ...
  ledger: {
    ...
    dineMode: {
      ...
      detailDesc: '按收款渠道分开填',
      fastField: '堂食总收入',
      fastHint: '当月堂食全部消费营收。含现金、微信、支付宝、刷卡、储值买单、挂账消费、团购核销。',
    ...
    incomeScope: {
      dine_in: '包括店内扫码点单、现金、微信支付宝、银行卡刷卡、储值卡核销、团购/代金券核销；个人/单位挂账、签单也按出餐当月计入。团购核销的钱结算在美团/抖音平台、不是当日到账，但仍按核销当月计入。不包括会员充值预收（没消费不算收入），也不包括收回挂账的回款（那是把应收款收回，不是营收）。',
